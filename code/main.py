...
def main():
    ...
    X = load_monash('kdd_cup_nomissing')['series_value']
    log = []
    for i, x in enumerate(X):
        print('-'*30, i, '-'*30)
        log = run_experiment(log, x, L, H, j, lr=2e-4, verbose=True)
...
def run_experiment(log, X, L, H, j, lr=1e-3, verbose=False):
    ...
    f_c = MLPRegressor((28,), random_state=1282781)
    f_c.fit(x_train.squeeze(), y_train.squeeze())
    preds_c = f_c.predict(x_val.squeeze()).squeeze()
    loss_c = mean_squared_error(preds_c, y_val.squeeze())
